...
Phase 4 当前推进状态（2026-04-14）：
1. `wip4-01` 已启动：`phase-4/README.md` 已从模板态补齐为执行级规划（对标结论、WIP 门禁、WP4-A0~E 设计核心、Case 矩阵）。
2. 新增 `wip4-01a / WP4-A0`：已进入 In Progress，先落地最小模式切换命令（`/mode`、`/plan`、`/implement`），降低 plan->implement 切换摩擦。
3. 对标发现已落盘：`IOrchestrator + NoopOrchestrator` 为已对齐基线；Plan/Task 状态机、汇聚协议、审批继承为主要差异项。
4. 执行切片已冻结：`PR4-0 ~ PR4-5` 对应 `wip4-01a ~ wip4-06`，按“最小切换 -> 状态机 -> 生命周期 -> 汇聚 -> 审批 -> 收口”顺序推进。
5. 阶段状态：`In Progress`（待 `wip4-01` 冻结后进入实现）。
