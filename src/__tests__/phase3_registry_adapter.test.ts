...
describe("WP3-C Registry Adapter", () => {
  test("REG-001: builtin and external capabilities are queryable in one registry", () => {
    ...
    expect(all.find((item) => item.source === "builtin" && item.name === "Read")?.callable).toBe(
      true,
    );
    expect(all.find((item) => item.source === "mcp" && item.name === "mcp.alpha")?.state).toBe(
      "ready",
    );
    ...
    adapter.rebuild({
      ...
      security: {
        guard: {
          mcpSnapshots: [],
          policy: {
            trustedMcpProviders: ["mcp.disabled"],
            trustedPlugins: ["plugin:disabled"],
          },
        },
      },
    });
    ...
    const summary = adapter.rebuild({
      ...
      security: {
        guard: {
          mcpSnapshots: [],
          policy: {
            trustedMcpProviders: ["mcp.search"],
